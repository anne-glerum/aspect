...
#include "plume.h"
#include "plume_only_boundary_T.h"
...
//        this->get_boundary_temperature_manager().template get_matching_boundary_temperature_model<BoundaryTemperature::PlumeOnly<dim> >();
      ...
      BoundaryTemperature::PlumeOnly<dim> *boundary_temperature;

      const std::vector<std_cxx11::shared_ptr<BoundaryTemperature::Interface<dim> > > &
      bvs = this->get_boundary_temperature_manager().get_active_boundary_temperature_conditions();

      for (typename std::vector<std_cxx11::shared_ptr<BoundaryTemperature::Interface<dim> > >::const_iterator
           p = bvs.begin();
           p != bvs.end(); ++p)
        {
            if (BoundaryTemperature::PlumeOnly<dim> *bt = dynamic_cast<BoundaryTemperature::PlumeOnly<dim> *>
                     (const_cast<BoundaryTemperature::Interface<dim> *>((*p).get())))
             {
               boundary_temperature = dynamic_cast<BoundaryTemperature::PlumeOnly<dim> *> (const_cast<BoundaryTemperature::Interface<dim> *>((*p).get()));
              break;
             }
        }
