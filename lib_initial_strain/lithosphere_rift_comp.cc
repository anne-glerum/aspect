...
namespace aspect
{
  namespace InitialComposition
  {
                         ...
                         const unsigned int compositional_index) const
    {
      ...
      if (depth < 0)
        return 0.;
      else if (depth <= local_upper_crust_thickness && compositional_index == id_upper)
        return 1.;
    ...
    LithosphereRift<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection ("Initial composition model");
      {
        ...
        {
          prm.declare_entry ("Sticky-air layer thickness", "0",
                             Patterns::Double (0),
                             "The thickness of a possible sticky-air layer. Units: m.");
