...
namespace aspect
{
  namespace InitialTopographyModel
  {
      ...
      for (unsigned int i_polygons=0; i_polygons<n_polygons; ++i_polygons)
        {
          ...
          // The total lithosphere thickness
          sum_polygon_thicknesses[i_polygons] =
            std::accumulate(polygon_thicknesses[i_polygons].begin(), polygon_thicknesses[i_polygons].end(),0);
    ...
    value (const Point<dim-1> &position) const
    {
      ...
      const std::vector<std::string> active_initial_composition_models =
        this->get_initial_composition_manager().get_active_initial_composition_names();
      AssertThrow(std::find(active_initial_composition_models.begin(),active_initial_composition_models.end(), "lithosphere with rift")
                  != active_initial_composition_models.end(),
      ...
      // When cartesian, position contains x(,y);
      // when spherical, position contains lon(,lat) (in degrees);
      ...
      const std::list<std_cxx11::shared_ptr<InitialComposition::Interface<dim> > > initial_composition_objects =
        this->get_initial_composition_manager().get_active_initial_composition_conditions();
      for (typename std::list<std_cxx11::shared_ptr<InitialComposition::Interface<dim> > >::const_iterator it = initial_composition_objects.begin();
           it != initial_composition_objects.end(); ++it)
    ...
    LithosphereRift<dim>::parse_parameters (ParameterHandler &prm)
    {
      ...
      prm.enter_subsection ("Initial composition model");
      {
        prm.enter_subsection("Lithosphere with rift");
        {
          ...
          A                      = Utilities::possibly_extend_from_1_to_N (Utilities::string_to_double
          ...
          for (unsigned int i_polygons = 0; i_polygons < n_polygons; ++i_polygons)
            {
              ...
              AssertThrow(polygon_thicknesses[i_polygons].size()==3,
                          ExcMessage ("The number of layer thicknesses should be equal to 3."));
            }
        }
        prm.leave_subsection();
      }
      ...
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Visco Plastic");
        {
          ...
          temp_densities = Utilities::possibly_extend_from_1_to_N
                           (Utilities::string_to_double(Utilities::split_string_list(prm.get("Densities"))),
                            n_fields+1,
                            "Densities");
        }
        prm.leave_subsection();
...
namespace aspect
{
  namespace InitialTopographyModel
  {
    ASPECT_REGISTER_INITIAL_TOPOGRAPHY_MODEL(LithosphereRift,
                                             ...
                                             "with rift' and should only be used in conjunction with this model. ")
  }
}
