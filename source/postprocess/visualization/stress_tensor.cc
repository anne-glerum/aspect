...
#include <aspect/postprocess/visualization/stress_tensor.h>
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      ...
      StressTensor ()
        :
        DataPostprocessorTensor<dim> ("stress_tensor",
        ...
        for (unsigned int q=0; q<n_quadrature_points; ++q)
          {
            ...
            for (unsigned int i=0; i<Tensor<2,dim>::n_independent_components; ++i)
              computed_quantities[q](i)
                = stress[stress.unrolled_to_component_indices(i)];
          }
      }
    }
  }
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      ASPECT_REGISTER_VISUALIZATION_POSTPROCESSOR(StressTensor,
                                                  "stress tensor",
                                                  "A visualization output object that generates output "
                                                  "for the 4 (in 2d) or 9 (in 3d) components of the stress "
