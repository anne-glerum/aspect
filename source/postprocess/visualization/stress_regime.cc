...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
            ...
            switch (dim)
              {
                ...
                case 3:
                {
                  ...
                  const Tensor<1,dim> S3 = eigen_vectors[dim].second;

                  // Compute the plunge and azimuth of the eigenvectors
                  // Azimuth is the angle of the horizontal projection of the vector
                  ...
                  // take the inverse cosine of the dot product divided by the lenght of the projected
                  // eigenvector.
                  Tensor<1,dim> north;
                  north[1] = 1.;
                  const double azimuth_S1 = acos(north * S1_hor);
                  const double azimuth_S2 = acos(north * S2_hor);
                  const double azimuth_S3 = acos(north * S3_hor);


                  // 3) compute plunge through the dot product of the projected and true
                  // eigenvector
                  const double plunge_S1 = acos(S1 * S1_hor / S1.norm());
                  const double plunge_S2 = acos(S2 * S2_hor / S2.norm());
                  const double plunge_S3 = acos(S3 * S3_hor / S3.norm());
                  ...
                  sigma_H[0] = std::cos(numbers::PI * 0.5 - sigma_H_azimuth);
                  sigma_H[1] = std::sin(numbers::PI * 0.5 - sigma_H_azimuth);
                  ...
                  if (stress_regime_old != stress_regime)
                    this->get_pcout() << "No match stress regimes new and old: " << stress_regime << " " << stress_regime_old << std::endl;
      ...
      StressRegime<dim>::eigenvectors(const SymmetricTensor<2, dim, Number> &T,
                   const SymmetricTensorEigenvectorMethod method) const
      {
        std::array<std::pair<Number, Tensor<1, dim, Number>>, dim> eig_vals_vecs = internal::SymmetricTensorImplementation::
