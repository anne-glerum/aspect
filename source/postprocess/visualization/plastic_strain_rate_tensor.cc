...
#include <aspect/postprocess/visualization/plastic_strain_rate_tensor.h>
...
namespace aspect
{
  namespace Postprocess
  {
    ...
    {
      template <int dim>
      PlasticStrainRateTensor<dim>::
      PlasticStrainRateTensor ()
        :
        DataPostprocessorTensor<dim> ("plastic_strain_rate_tensor",
                            ...
                            std::vector<Vector<double>> &computed_quantities) const
      {
        AssertThrow(this->get_parameters().enable_elasticity == true,
                    ExcMessage("Visualization plugin ``plastic strain rate tensor'' only works if 'Enable elasticity' is set to true."));
        ...
        for (unsigned int q=0; q<n_quadrature_points; ++q)
          {
            // Get the plastic strain rate from the material model.
            ...
            Assert(elastic_additional_out != nullptr, ExcMessage("Elastic Additional Outputs are needed for the ``plastic strain rate tensor'' postprocessor, but they have not been created."));

            const SymmetricTensor<2,dim> plastic_strain_rate = elastic_additional_out->plastic_strain_rate[q];

            for (unsigned int d=0; d<dim; ++d)
              for (unsigned int e=0; e<dim; ++e)
                computed_quantities[q][Tensor<2,dim>::component_to_unrolled_index(TableIndices<2>(d,e))]
                  = plastic_strain_rate[d][e];
          }
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      ASPECT_REGISTER_VISUALIZATION_POSTPROCESSOR(PlasticStrainRateTensor,
                                                  "plastic strain rate tensor",
                                                  "A visualization output object that generates output "
                                                  "for the 3 (in 2d) or 6 (in 3d) components of the plastic "
                                                  "strain rate tensor."
                                                  "\n\n"
