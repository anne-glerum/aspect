...
#include <aspect/postprocess/free_surface_BC.h>
...
namespace aspect
{
  namespace Postprocess
  {
    ...
    FreeSurfaceBC<dim>::execute (TableHandler &statistics)
    {
      ...
      if (GeometryModel::Sphere<dim> *gm = dynamic_cast<GeometryModel::Sphere<dim> *>
        ...
        {
          AssertThrow(false, ExcMessage("FreeSurfaceBC postprocessor is not yet implemented "
            ...
            if (cell->face(face_no)->at_boundary())
              {
                ...
                for (unsigned int corner = 0; corner < quadrature_formula_face.size(); ++corner)
                  {
                    const Point<dim> vertex = fe_face_vals.quadrature_point(corner);
                    const double elevation = this->get_geometry_model().height_above_reference_surface(vertex);
                    const Tensor<1,dim> velocity =  velocity_vals[corner];
                    const Tensor<1,dim> direction = fe_face_vals.normal_vector(corner);
                    const Tensor<1,dim> BC = (velocity * direction) * direction;
                    if (write_to_file)
                      output_file << vertex << ' ' << velocity[0] << ' ' << velocity[1] << ' ' << velocity[dim-1] <<
                                               ' ' << BC[0] << ' ' << BC[1] << ' ' << BC[dim-1] << std::endl;
      ...
      const double max_free_surface_BC = Utilities::MPI::max(local_max_height, this->get_mpi_communicator());
      const double min_free_surface_BC = Utilities::MPI::min(local_min_height, this->get_mpi_communicator());

      //Write results to statistics file
      statistics.add_value ("Minimum free_surface_BC (m)",
                            min_free_surface_BC);
      statistics.add_value ("Maximum free_surface_BC (m)",
                            max_free_surface_BC);
      const char *columns[] = { "Minimum free_surface_BC (m)",
                                "Maximum free_surface_BC (m)"
                              };
      ...
      output_stats << min_free_surface_BC << " m, "
                   << max_free_surface_BC << " m";
      ...
      if (!write_to_file || ((this->get_time() < last_output_time + output_interval)
        ...
        return std::pair<std::string, std::string> ("FreeSurfaceBC min/max:",
                                                    output_stats.str());
    ...
    template <int dim>
    void
    FreeSurfaceBC<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        prm.enter_subsection("FreeSurfaceBC");
        {
          prm.declare_entry ("Output to file", "false",
                             Patterns::List(Patterns::Bool()),
                             "Whether or not to write free_surface_BC to a text file named named "
                             "'free_surface_BC.NNNNN' in the output directory");
    ...
    template <int dim>
    void
    FreeSurfaceBC<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        prm.enter_subsection("FreeSurfaceBC");
...
namespace aspect
{
  namespace Postprocess
  {
    ASPECT_REGISTER_POSTPROCESSOR(FreeSurfaceBC,
                                  "free surface BC",
                                  ...
                                  "maximum and minimum free_surface_BC relative to a reference datum (initial "
                                  "box height for a box geometry model or initial radius for a "
                                  "sphere/spherical shell geometry model). "
                                  "If 'FreeSurfaceBC.Output to file' is set to true, also outputs free_surface_BC "
                                  "into text files named `free_surface_BC.NNNNN' in the output directory, "
                                  "where NNNNN is the number of the time step.\n"
                                  "The file format then consists of lines with Euclidean coordinates "
                                  "followed by the corresponding free_surface_BC value."
                                  "FreeSurfaceBC is printed/written in meters.")
  }
}
