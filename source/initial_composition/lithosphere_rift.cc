...
namespace aspect
{
  namespace InitialComposition
  {
    ...
    initialize ()
    {
      ...
      AssertThrow(find(active_initial_temperature_models.begin(),active_initial_temperature_models.end(), "lithosphere with rift")
                  != active_initial_temperature_models.end(),
                         ...
                         const unsigned int compositional_index) const
    {
      ...
      const double local_upper_crust_thickness = ((0.5+0.5*std::tanh(distance_to_L_polygon.first/sigma_polygon))
                                                  *polygon_thicknesses[distance_to_L_polygon.second][0]
                                                  +(0.5-0.5*std::tanh(distance_to_L_polygon.first/sigma_polygon))*thicknesses[0])
                                                 * (!blend_rift_and_polygon && distance_to_L_polygon.first > 0.-2.*sigma_polygon
                                                    ? 1. : (1.0 - A[0] * std::exp((-std::pow(distance_to_rift_axis,2)/(2.0*std::pow(sigma_rift,2))))));
      const double local_lower_crust_thickness = ((0.5+0.5*std::tanh(distance_to_L_polygon.first/sigma_polygon))
                                                  *polygon_thicknesses[distance_to_L_polygon.second][1]
                                                  +(0.5-0.5*std::tanh(distance_to_L_polygon.first/sigma_polygon))*thicknesses[1])
                                                 * (!blend_rift_and_polygon && distance_to_L_polygon.first > 0.-2.*sigma_polygon
                                                    ? 1. : (1.0 - A[1] * std::exp((-std::pow(distance_to_rift_axis,2)/(2.0*std::pow(sigma_rift,2))))));
      const double local_mantle_lithosphere_thickness = ((0.5+0.5*std::tanh(distance_to_L_polygon.first/sigma_polygon))
                                                         *polygon_thicknesses[distance_to_L_polygon.second][2]
                                                         +(0.5-0.5*std::tanh(distance_to_L_polygon.first/sigma_polygon))*thicknesses[2])
                                                        *  (!blend_rift_and_polygon && distance_to_L_polygon.first > 0.-2.*sigma_polygon
                                                            ? 1. : (1.0 - A[2] * std::exp((-std::pow(distance_to_rift_axis,2)/(2.0*std::pow(sigma_rift,2))))));
      ...
      if (depth <= local_upper_crust_thickness && compositional_index == id_upper)
        return 1.;
      else if (depth > local_upper_crust_thickness
               && depth <= local_upper_crust_thickness+local_lower_crust_thickness && compositional_index == id_lower)
        return 1.;
      else if (depth > local_upper_crust_thickness+local_lower_crust_thickness &&
               depth <= local_upper_crust_thickness+local_lower_crust_thickness+local_mantle_lithosphere_thickness &&
    ...
    distance_to_rift (const Point<dim-1> &surface_position) const
    {
      ...
      for (unsigned int i_segments = 0; i_segments < point_list.size(); ++i_segments)
        {
          temp_distance = (dim == 2) ? std::abs(surface_position[0]-point_list[i_segments][0][0])
                          : std::abs(Utilities::distance_to_line(point_list[i_segments], Point<2>(surface_position[0],surface_position[dim-2])));
          ...
          if (dim == 2)
            {
              ...
              if (surface_position[0]>polygon_point_list[n][0][0] && surface_position[0]<polygon_point_list[n][1][0])
                sign = 1.;
              temp_distance = sign * std::min(std::abs(polygon_point_list[n][1][0]
                                                       - surface_position[0]), std::abs(surface_position[0] - polygon_point_list[n][0][0]));
            }
          else
            {
              temp_distance = Utilities::signed_distance_to_polygon<dim>
                              (polygon_point_list[n], Point<2>(surface_position[0],surface_position[dim-2]));
            }
      ...
      prm.enter_subsection ("Initial composition model");
      {
        prm.enter_subsection("Lithosphere with rift");
        {
          ...
          prm.declare_entry ("Half width of polygon smoothing", "20000",
                             Patterns::Double (0),
                             "The half width of the hyperbolic tangent smoothing used "
                             "to transition to the lithospheric thicknesses of the polygon. "
                             "Note that this parameter is taken to be the same for all polygons. "
                             "Units: $m$ or degrees.");
          prm.declare_entry ("Blend polygons and rifts", "true",
                             Patterns::Bool (),
                             "Whether or not to blend the contributions of polygons and the rift. "
                             "For true, they're blended together. "
          ...
          prm.declare_entry ("Lithospheric polygon layer thicknesses", "30000.",
                             ...
                             "For each polygon a total of 3 values should be given "
    ...
    LithosphereRift<dim>::parse_parameters (ParameterHandler &prm)
    {
      ...
      prm.enter_subsection ("Initial composition model");
      {
        prm.enter_subsection("Lithosphere with rift");
        {
          ...
          A                      = Utilities::possibly_extend_from_1_to_N
                                   (Utilities::string_to_double
                                    (Utilities::split_string_list(prm.get("Amplitude of Gaussian rift geometry"))),
                                    3,
                                    "Amplitude of Gaussian rift geometry");
          thicknesses = Utilities::possibly_extend_from_1_to_N
                        (Utilities::string_to_double(Utilities::split_string_list(prm.get("Layer thicknesses"))),
                         3,
                         "Layer thicknesses");
          ...
          for (unsigned int i_segment = 0; i_segment < n_temp_segments; i_segment++)
            {
              ...
              if (dim == 3)
                {
                  AssertThrow(temp_segment.size() == 2,
                              ExcMessage ("The given coordinate '" + temp_segment[i_segment] + "' is not correct. "
                                          "It should only contain 2 parts: "
                                          "the two points of the segment, separated by a '>'."));
                }
              else
                {
                  AssertThrow(temp_segment.size() == 1,
                              ExcMessage ("The given coordinate '" + temp_segment[i_segment] + "' is not correct. "
              ...
              for (unsigned int i_points = 0; i_points < dim-1; i_points++)
                {
                  const std::vector<double> temp_point =
                    Utilities::string_to_double(Utilities::split_string_list(temp_segment[i_points],','));
                  if (dim == 3)
                    {
                      AssertThrow(temp_point.size() == 2,
                                  ExcMessage ("The given coordinates of segment '" + temp_segment[i_points] + "' are not correct. "
                                              "It should only contain 2 parts: "
                                              "the two coordinates of the segment end point, separated by a ','."));
                    }
                  else
                    {
                      AssertThrow(temp_point.size() == 1,
                                  ExcMessage ("The given coordinates of segment '" + temp_segment[i_points] + "' are not correct. "
                                              "It should only contain 1 part: "
          ...
          const std::vector<std::string> temp_thicknesses =
            Utilities::split_string_list(prm.get("Lithospheric polygon layer thicknesses"),';');
          ...
          for (unsigned int i_polygons = 0; i_polygons < n_polygons; ++i_polygons)
            {
              polygon_thicknesses[i_polygons] =
                Utilities::string_to_double(Utilities::split_string_list(temp_thicknesses[i_polygons],','));
              AssertThrow(polygon_thicknesses[i_polygons].size()==3,
                          ExcMessage ("The number of layer thicknesses should be equal to 3 for polygon: "
                                      + Utilities::int_to_string(i_polygons) +
              ...
              if (dim == 3)
                {
                  AssertThrow(n_temp_points>=3,
                              ExcMessage ("The number of polygon points should be equal to or larger than 3 in 3d."));
                }
              else
                {
                  AssertThrow(n_temp_points==2,
                              ExcMessage ("The number of polygon points should be equal to 2 in 2d."));
                }
              ...
              for (unsigned int i_points = 0; i_points < n_temp_points; i_points++)
                {
                  const std::vector<double> temp_point =
                    Utilities::string_to_double(Utilities::split_string_list(temp_points[i_points],','));
                  AssertThrow(temp_point.size() == dim-1,
                              ExcMessage ("The given coordinates of point '" + temp_points[i_points] + "' are not correct. "
              ...
              if  (dim == 2)
                AssertThrow(polygon_point_list[i_polygons][0][0] < polygon_point_list[i_polygons][1][0],
                            ExcMessage("The order of the x coordinates of the 2 points "
