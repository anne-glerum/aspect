...
namespace aspect
{
  namespace MeshRefinement
  {
    ...
    initialize ()
    {
      ...
      const std::vector<std::string> active_initial_temperature_models =
        this->get_initial_temperature_manager().get_active_initial_temperature_names();
      AssertThrow(std::find(active_initial_temperature_models.begin(),active_initial_temperature_models.end(), "lithosphere with rift")
      ...
      prm.enter_subsection ("Mesh refinement");
      {
        prm.enter_subsection("Lithosphere with rift");
        {
          prm.declare_entry ("Width of refined area along rift", "100000",
                             ...
                             "This area is centered around the rift axis, such that "
                             "the refined distance from the center is width/2."
    ...
    LithosphereRift<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Mesh refinement");
      {
        ...
        rift_refinement_level = prm.get_integer ("Initial global refinement")
