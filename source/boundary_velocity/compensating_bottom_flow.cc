    ...
    CompensatingBottomFlow<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection ("Boundary velocity model");
      {
        prm.enter_subsection ("Compensating bottom flow model");
        {
          ...
          prm.declare_entry ("Vertically compensate net flow", "false",
                             Patterns::Bool (),
          ...
          prm.declare_entry ("Zero horizontal velocities at inner radius", "true",
                             Patterns::Bool (),
                             "Whether to phase out the horizontal velocities towards the lower boundary, "
                             "or towards the bottom of the velocity transition zone, with the horizontal velocities "
                             "being zero below this transition zone. ");
          prm.declare_entry ("Zero radial velocities at lateral boundaries", "true",
                             Patterns::Bool (),
                             "Whether to prescribe zero radial velocities on the lateral boundaries, "
                             "or to prescribe radial velocities that decrease to zero at the bottom of the "
                             "transition zone. ");
    ...
    CompensatingBottomFlow<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Boundary velocity model");
      {
        prm.enter_subsection("Compensating bottom flow model");
        {
          ...
          vertical_residual_compensation = prm.get_bool ("Vertically compensate net flow");
          zero_horizontal_velocities_at_inner_radius = prm.get_bool ("Zero horizontal velocities at inner radius");
          zero_radial_velocities_at_lateral_boundaries = prm.get_bool ("Zero radial velocities at lateral boundaries");
