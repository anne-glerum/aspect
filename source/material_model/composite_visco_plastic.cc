...
#include <aspect/material_model/composite_visco_plastic.h>
...
namespace aspect
{
  namespace MaterialModel
  {
             ...
             MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      ...
      std::vector<double> inverse_kelvin_viscosities(in.n_evaluation_points(), 0.0);
      ...
      for (unsigned int i=0; i < in.n_evaluation_points(); ++i)
        {
          ...
          if (in.requests_property(MaterialProperties::viscosity) || in.requests_property(MaterialProperties::additional_outputs))
            {
              inverse_kelvin_viscosities[i] = rheology->compute_inverse_kelvin_viscosity(volume_fractions);
              SymmetricTensor<2,dim> effective_strain_rate = in.strain_rate[i];
              if (this->get_parameters().enable_elasticity)
                {
                  ...
                  const SymmetricTensor<2,dim> elastic_stress = (Utilities::Tensors::to_symmetric_tensor<dim>(&in.composition[i][stress_field_indices[0]],
                                                                 &in.composition[i][stress_field_indices[0]]+SymmetricTensor<2, dim>::n_independent_components));
                  effective_strain_rate = rheology->compute_effective_strain_rate(in.strain_rate[i], elastic_stress, inverse_kelvin_viscosities[i]);
                }
              std::vector<double> partial_strain_rates(6, 0.);
              // TODO get real grain size
              out.viscosities[i] = rheology->compute_viscosity(in.pressure[i], in.temperature[i], 1e-3, volume_fractions, effective_strain_rate, inverse_kelvin_viscosities[i], partial_strain_rates);

              // TODO update
              plastic_yielding = false;
            }
          else
            {
      ...
      if (this->get_parameters().enable_elasticity)
        {
          ...
          rheology->fill_elastic_outputs(in, inverse_kelvin_viscosities, out);
          ...
          rheology->fill_elastic_additional_outputs(in, inverse_kelvin_viscosities, out);
          // Fill the reaction terms that account for the rotation of the stresses.
          rheology->fill_reaction_outputs(in, inverse_kelvin_viscosities, out);
          ...
          // to obtain $\tau^{t}$.
          rheology->fill_reaction_rates(in, inverse_kelvin_viscosities, out);
        }
    }
    ...
    template <int dim>
    double CompositeViscoPlastic<dim>::
    get_min_strain_rate () const
    {
      return rheology->minimum_strain_rate;
    }
    ...
    CompositeViscoPlastic<dim>::declare_parameters (ParameterHandler &prm)
    {
      ...
      {
        prm.enter_subsection ("Composite Visco Plastic");
        {
    ...
    {
      if (this->get_parameters().enable_elasticity)
        rheology->create_elastic_additional_outputs(out);
    }
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(CompositeViscoPlastic,
                                   "composite visco plastic",
