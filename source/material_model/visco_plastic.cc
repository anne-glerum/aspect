...
namespace aspect
{
  namespace MaterialModel
  {
                         ...
                         MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      ...
      // copy pasted all these lines from above to have the correct current friction angle as an output
          ...
          for (unsigned int j=0; j < volume_fractions.size(); ++j)
            {
              ...
              plastic_out->friction_angles[i] += 180.0/numbers::PI * volume_fractions[j] * (dynamic_angles_of_internal_friction[j]
                                                                                            + (drucker_prager_parameters.angles_internal_friction[j] * weakening_factors[1]
                                                                                                - dynamic_angles_of_internal_friction[j])
                                                                                            / (1 + std::pow((current_edot_ii / dynamic_characteristic_strain_rate[j]),
                                                                                                dynamic_friction_smoothness_exponent)));
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(ViscoPlastic,
                                   ...
                                   "\n\n "
                                   "When dynamic angles of friction are specified, the friction angle will be "
                                   "weakened for high strain rates with: "
                                   ...
                                   "It can be used to simulate earthquake-like behaviour, where short periods of "
                                   "high-velocities are seperated by longer periods without movement.  "
                                   "\n\n "
