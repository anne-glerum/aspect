  ...
  namespace MaterialModel
  {
    ...
    namespace Rheology
    {
                             ...
                             const Point<dim> &position) const
      {
              ...
              if (current_cell.state() == IteratorState::valid)
                {
                  ...
                  const double rate_and_state_parameter_a = calculate_depth_dependent_a_and_b(position,volume_fraction_index).first;
                  const double rate_and_state_parameter_b = calculate_depth_dependent_a_and_b(position,volume_fraction_index).second;
                  ...
                  double stst_RSF_friction = std::atan (mu);

                  return stst_RSF_friction;
                }
              break;
            }
              ...
              if (current_cell.state() == IteratorState::valid)
                {
                  ...
                  // Get the values for a and b and the critcal slip distance L
                  double rate_and_state_parameter_a = calculate_depth_dependent_a_and_b(position,volume_fraction_index).first;
                  const double rate_and_state_parameter_b = calculate_depth_dependent_a_and_b(position,volume_fraction_index).second;
                  ...
                  double mu = 0;
                  if (friction_mechanism == regularized_rate_and_state_dependent_friction)
                    {
                      ...
                      if (rate_and_state_parameter_a > 0)
                        ...
                        mu = std::tan(static_friction_angle) + rate_and_state_parameter_b
                             * std::log((theta * RSF_ref_velocity) / critical_slip_distance);
                    }
                  else
                    {
                      ...
                      if (friction_mechanism == rate_and_state_dependent_friction_plus_linear_slip_weakening)
                      {
        ...
        /** ToDo: I added this assert in the PRs moving the dynamic friction rheology and adding a friction function.
         *  But now with the RSF I am not sure where it belongs ... Maybe it is not needed anymore because I have a default now? */
      ...
      FrictionModels<dim>::calculate_depth_dependent_a_and_b(const Point<dim> &position, const int volume_fraction_index) const
      {
        ...
        const double rate_and_state_parameter_a =
          rate_and_state_parameter_a_function->value(Utilities::convert_array_to_point<dim>(point.get_coordinates()),volume_fraction_index);
        const double rate_and_state_parameter_b =
          rate_and_state_parameter_b_function->value(Utilities::convert_array_to_point<dim>(point.get_coordinates()),volume_fraction_index);
      ...
      template <int dim>
      double
      FrictionModels<dim>::get_critical_slip_distance(const Point<dim> &position, const int volume_fraction_index) const
      {
        Utilities::NaturalCoordinate<dim> point =
          this->get_geometry_model().cartesian_to_other_coordinates(position, coordinate_system_RSF);

        const double critical_slip_distance =
          critical_slip_distance_function->value(Utilities::convert_array_to_point<dim>(point.get_coordinates()),volume_fraction_index);

        if (use_theta() && (volume_fraction_index > 0) && RSF_composition_masks[volume_fraction_index-1])
      ...
      template <int dim>
      double
      FrictionModels<dim>::get_effective_friction_factor(const Point<dim> &position) const
      {
        Utilities::NaturalCoordinate<dim> point =
          this->get_geometry_model().cartesian_to_other_coordinates(position, coordinate_system_RSF);

        // if I substract the input value from 1, the default for function values, being 0, leads to a factor of 1,
        // so the pressure / friction angle is not modified, which is an appropriate default, but a bit annoying as
      ...
      use_theta() const
      {
        ...
        const FrictionMechanism mechanism = get_friction_mechanism();
      ...
      FrictionModels<dim>::parse_parameters (ParameterHandler &prm)
      {
        ...
        /* ToDo: These exceptions are maybe still valid if theta is not computed on particles! check with new VEP!
        if (use_theta())
          {
        ...
        if (out.template get_additional_output<FrictionAdditionalOutputs<dim> >() == nullptr)
          {
            ...
            out.additional_outputs.push_back(
              std::make_unique<FrictionAdditionalOutputs<dim>> (n_points));
          }
      }
