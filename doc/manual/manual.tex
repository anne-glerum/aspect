...
\subsubsection{2D subduction}
\label{sec:cookbooks-2d-subduction}
\textit{This section was contributed by Anne Glerum}

Contrary to the continental extension and mid-ocean-ridge models, at convergent plate boundaries plates move towards each other and possibly one subducts underneath the other. To model this process of subduction, three ingredients are important:
\begin{itemize}
  \item A subducting and overriding plate that are distinguished from the mantle through their high viscosity. 
  \item The forces driving subduction. We can prescribe a certain velocity to the subducting plate to force subduction, or let subduction evolve self-consistently from the negatively-buoyant slab tip extending into the mantle.
  \item A way for the subducting plate to decouple from the surface and the overriding plate. A sticky-air or weak crustal layer facilitates the bending of the plate, while a weak zone between the subducting and overriding plate is needed to prevent locking of the subduction interface. This weak zone can coincide with the crust. 
\end{itemize}

With these two simple ingredients, a simple subduction model can already be set up
